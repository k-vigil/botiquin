    ...
    <div class="mb-3">
        <div class="row">
            <div class="col-6">
                ...
                <h5 class="font-weight-semibold">Laboratorios</h5>
            </div>
...
<!-- modal para registrar laboratorio -->
    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content border-light shadow-sm">
            <div class="modal-header border-0">
                <h5 class="modal-title font-weight-semibold" id="modalRegistrarLabel">Nuevo laboratorio</h5>
...
<div class="modal" id="modalEditar" tabindex="-1" aria-labelledby="modalEditarLabel" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content border-light shadow-sm">
            <div class="modal-header border-0">
                <h5 class="modal-title font-weight-semibold" id="modalEditarLabel">Editar laboratorio</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>
    ...
    <div class="modal-dialog modal-dialog-centered mx-auto" style="width: 320px;">
        <div class="modal-content border-0 shadow-sm px-1">
            <div class="modal-header">
                <h5 class="modal-title font-weight-semibold" id="modalEliminarLabel">Eliminar laboratorio</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>
...
@push('scripts')
<script>
    document.getElementById('laboratorios').classList.add('active')
</script>
<script>
            ...
            <tr>
                ...
                <td class="text-right" style="width: 80px;">
                    <div class="btn-group align-items-center">
                        <button type="button" class="btn btn-link px-1 py-0" data-toggle="modal" data-target="#modalEditar" onClick="obtenerLaboratorioParaEditar(${i.id})">Editar</button>
                        <button type="button" class="btn btn-link px-1 py-0" data-toggle="modal" data-target="#modalEliminar" onClick="obtenerLaboratorioParaEliminar(${i.id})">Borrar</button>
                    </div>
                </td>
            </tr>
        `);
    ...
    async function registrarLaboratorio(body) {
        try {
            const res = await http.post('/laboratorios', body)
            const {
                data
            } = res
    ...
    async function editarLaboratorio(body) {
        try {
            const res = await http.put(`/laboratorios/${body.id}`, body)
            const {
                data
            } = res
            ...
            Toast.fire({
                ...
                text: "Laboratorio actuzalizado exitosamente"
            })
            listarLaboratorios()
        } catch (err) {
            Toast.fire({
    ...
    async function obtenerLaboratorioParaEditar(id) {
        console.log(id);

        try {
            const res = await http.get(`/laboratorios/${id}`)
            const {
                data
            } = res
    ...
    async function obtenerLaboratorioParaEliminar(id) {
        console.log(id);
    ...
    async function eliminarLaboratorio(id) {
        try {
            const res = await http.delete(`/laboratorios/${id}`)
            const {
                data
            } = res
    ...
    document.getElementById('form-registrar').addEventListener('submit', async function(e) {
        ...
        await registrarLaboratorio(body)
